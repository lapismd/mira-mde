...
| MIRA-ARCH-021 | The shipped `@lapismd/mira/preview` implementation MUST retain portable semantic icons requested by consumer frontmatter type definitions, including the built-in Tags definition's canonical Lucide hash geometry, and its frontmatter editor's narrow single-column container behavior without requiring consumer Tailwind processing or an app-specific icon registry.                                                                        |
