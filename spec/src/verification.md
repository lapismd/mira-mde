...
| MIRA-MD-010                                                                                             | Scroll-root unit coverage plus focused owner and linked Lapis active-section browser acceptance                                                                                  | Implemented                                                                                        |
...
| MIRA-CSS-016                                                                                            | Shipped source-table selector plus focused computed-style and per-line hidden-chrome overflow and same-table scroll-sync acceptance                                              | Implemented by table parity slice                                                                  |
...
| MIRA-MERMAID-006                                                                                        | Mermaid package manifest `^11.16.1` range plus root `pnpm audit`                                                                                                                 | Implemented                                                                                        |
...
| MIRA-GOV-009, MIRA-GOV-010, MIRA-GOV-011                                                                | Changeset intent gate, Changesets YAML reader test, npm integrity response normalization, protected release workflow, post-publish checks, and package GitHub release-note tests | Implemented                                                                                        |
| MIRA-GOV-013                                                                                            | `AGENTS.md` published dependency policy and portable manifest boundary                                                                                                           | Implemented                                                                                        |
| MIRA-GOV-014                                                                                            | Release-workflow checker and regression tests for Node.js 24 action majors, generated release PR gating, and spec-first CLI arguments                                            | Implemented                                                                                        |
| MIRA-GOV-015                                                                                            | Release-workflow checker requires `pnpm audit` in pull-request CI and release revalidation                                                                                       | Implemented                                                                                        |
| MIRA-ARCH-039                                                                                           | Root `check`/`check:all` scripts, major-bounded workspace overrides, Svelte dev-range audit refresh, Changesets YAML reader test, and `pnpm audit`                               | Implemented                                                                                        |
...
| MIRA-MD-037                                                                                             | Frontmatter value-suggestion helpers plus pill-input autocomplete; optional FrontmatterEditor fileAdapter context                                                                | Implemented by value suggestion hook                                                               |
| MIRA-ARCH-035                                                                                           | Public preview frontmatter types and FrontmatterEditor accept valueSuggestions and fileAdapter                                                                                   | Implemented by value suggestion hook                                                               |
| MIRA-CSS-038                                                                                            | Shipped frontmatter CSS positions value-suggestion popovers like property-name suggestions                                                                                       | Implemented by value suggestion hook                                                               |
| MIRA-MD-038, MIRA-CSS-039, MIRA-ARCH-036                                                                | Mira UI source contract rejects unscoped generic tooltip-content paint and stacking selectors                                                                                    | Implemented by tooltip ownership repair                                                            |
| MIRA-MD-039                                                                                             | Live-preview hidden-formatting test keeps ordinary parentheses visible and still hides real link closers                                                                         | Implemented by live-preview delimiter repair                                                       |
| MIRA-CSS-040                                                                                            | Focused table story wraps live-preview cells with no cell overflow and scrolls raw source lines without chrome                                                                   | Implemented by live-preview table wrap                                                             |
| MIRA-MD-040, MIRA-CSS-041, MIRA-ARCH-037                                                                | Portable component interaction, shipped CSS contract, and focused Mira and linked Lapis Storybook geometry assertions                                                            | Implemented by frontmatter type-menu visibility repair                                             |
| MIRA-MD-041, MIRA-ARCH-038                                                                              | Portable FrontmatterEditor regression plus linked Lapis File Properties interaction acceptance                                                                                   | Implemented by external frontmatter text reconciliation                                            |
...
order and marks the run as requiring manual first publication; preparation
produces six verified tarballs, while a local publish attempt stops at the
approved-CI guard. The release workflow no longer accepts a temporary npm
bootstrap token and the Storybook Pages workflow publishes only the validated
static build artifact. Static workflow validation covers release, CI, and Pages.
...
repository, protected `npm-production` and `github-pages` environments, manual
first publication, and six npm trusted-publisher registrations described in
...
focused Chromium acceptance owns the browser geometry assertions. Active-prefix
blur/refocus coverage asserts stable glyph columns and wrapped row positions
rather than raw padding variables, because equivalent CodeMirror layout states
may rebalance padding and text-indent without shifting visible content.
