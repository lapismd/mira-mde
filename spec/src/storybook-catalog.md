...
| MIRA-CAT-005 | Every testable public story MUST render in the Storybook browser test project; meaningful stateful behavior MUST use a `play` interaction. The Storybook host MUST resolve CodeMirror and Lezer peer packages through one canonical Mira workspace instance so parser, state, view, and language-support identities remain compatible across stories and first-party plugins.                                       |
...
| MIRA-CAT-014 | Storybook MUST catalog `MiraCodeEditor` with an interactive language-extension example that proves the shell remains language-neutral, exposes an accessible textbox, accepts edits, and supports both framed content-height and frameless fill-height presentations. Its token assignment MUST include the inheritable shared editor, gutter, search input/button states, focus-ring, and syntax palette contract. |
| MIRA-CAT-015 | The focused `MiraCodeEditor` catalog MUST open the real Find interface and verify that the three query options are rendered inside the Find field while search navigation, accessible option state, compact shell-owned input and placeholder typography, and the editor-owned transparent panel surface remain available.                                                                                          |
