...
| MIRA-GOV-009 | A protected public-package change MUST include a Changeset or an explicit empty Changeset, and the Version Packages workflow MUST preserve independent package versions and per-package changelogs. Release tests MUST prove Changesets can still read workspace YAML after workspace audit pins, without requiring a local `main` branch.                                                              |
...
integrity response normalization and disagreement, scoped npm provenance PURLs, release-intent
...
Packages pull request without receiving npm credentials. Release tests invoke
Changesets' `read-yaml-file` reader directly so pull-request checkouts without
a local `main` still prove `pnpm-workspace.yaml` parses after `js-yaml` audit
pins, which MUST stay on the requested major line so `safeLoad` remains
available. Only a commit with no pending Changesets advances to artifact
...
All repository workflows configure Node.js 24 and Node.js 24-compatible action
majors. The generated Changesets version pull request does not repeat
source-change governance; ordinary pull requests still compare their exact base
and head revisions through the validator's native argument parser.
