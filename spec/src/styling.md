...
| MIRA-CSS-018 | The editable list-highlight trigger and portaled picker MUST use shipped semantic hooks and theme tokens. Trigger and option buttons MUST use a transparent ghost treatment at rest, adding their background and border only while hovered, keyboard-focused, or open. The trigger MUST preserve the existing rendered marker footprint, color, and custom-marker renderer without moving the list content column. The picker MUST show each resolved marker in its configured color and expose a visible selected state. |
