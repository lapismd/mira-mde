...
| MIRA-CSS-018 | The editable list-highlight trigger and portaled picker MUST use shipped semantic hooks and theme tokens. Trigger and option buttons MUST use a transparent ghost treatment at rest, adding their background and border only while hovered, keyboard-focused, or open. The trigger MUST use padding rather than margin for inline separation, keep its visual border clear of the list bullet, and preserve a compact gap before item content without moving the content column. Text and icon markers MUST occupy the same centered marker slot on the first visual row, including when the item wraps, while retaining the configured color and custom-marker renderer. Consecutive list-highlight backgrounds MUST be separated without applying margins to CodeMirror lines or widget roots. The picker MUST show each resolved marker in its configured color and expose a visible selected state. |
