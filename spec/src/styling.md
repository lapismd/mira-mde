...
| MIRA-CSS-018 | The editable list-highlight trigger and portaled picker MUST use shipped semantic hooks and theme tokens. The trigger MUST preserve the existing rendered marker footprint, color, and custom-marker renderer while adding hover and keyboard-focus affordances without moving the list content column. The picker MUST show each resolved marker in its configured color and expose a visible selected state. |
