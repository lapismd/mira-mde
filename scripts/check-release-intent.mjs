...
const PUBLIC_DIRECTORIES = new Set(
  PUBLIC_PACKAGE_GRAPH.map(({ directory }) => directory),
);
...
export function validateReleaseIntent(files) {
  ...
  const publicChanges = normalized.filter((file) => {
    ...
    if (!match || !PUBLIC_DIRECTORIES.has(match[1])) return false;
    const packagePath = match[2];
    if (packagePath === "CHANGELOG.md") return false;
    if (/\.(?:spec|test)\.[cm]?[jt]sx?$/.test(packagePath)) return false;
    if (/\.stories\.[cm]?[jt]sx?$/.test(packagePath)) return false;
    return packagePath === "package.json" || packagePath.startsWith("src/");
  });
...
function changedFiles(options) {
  ...
  return run("jj", ["--no-pager", "diff", "--name-only", "-r", "@"]);
}
