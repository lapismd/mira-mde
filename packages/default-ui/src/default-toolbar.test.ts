...
async function renderToolbar(props: Partial<MiraDefaultToolbarProps> = {}) {
  const target = document.createElement("div");
  document.body.append(target);
  const component = mount(MiraDefaultToolbar, {
    target,
    props,
  });
  ...
  return {
    target,
    separators() {
      return target.querySelectorAll(".mira-default-ui__separator");
    },
    destroy() {
      unmount(component);
      target.remove();
