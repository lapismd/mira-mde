...
function enterTable(token) {
  ...
  this.data.tableInfo = {
    ...
    cells: [],
    // grid dividers use for align the cells
    dividers: [],
...
function createExitTable(options) {
  ...
  return function exitTable(token) {
    ...
    for (const cell of tableInfo.allCells) {
      const { node, lines, colSpan, rowSpan, align, valign } = cell;
      ...
      node.content = cellContent;
      node.children = tree.children;
      node.position = {
        start: pick(cell.startPosition, ["line", "column", "offset"]),
        end: pick(cell.endPosition, ["line", "column", "offset"]),
      };
      node.positions = cell.positions;
      if (colSpan > 1) {
...
function exitCell(token) {
  ...
  if (tableInfo.isDivider) {
    ...
    if (cell) {
      cell.rowSpan += 1;
      // Update end position for rowspan cells - always extend to current token end
      if (token.end) {
        cell.endPosition = token.end;
      }
    }
  }
  ...
  if (cell) {
    ...
    cell.positions.push({ from: token.start.offset, to: token.end.offset });
    // Always update end position to reflect the latest content
    if (token.end) {
      cell.endPosition = token.end;
    }
  ...
  if (!cell) {
    ...
    cell = {
      ...
      lines: [],
      positions: [],
      // Store the original token positions
    ...
    // Cell already exists (multi-line), update end position
    if (token.end) {
      cell.endPosition = token.end;
    }
  }
...
function enterRowLine(token) {
  ...
  // Store row start position for later use
  if (token.start) {
    info.rowStart = token.start;
  }
}

function commitRow(info) {
  // Calculate row boundaries from cells
  let rowStart = info.rowStart;
  let rowEnd = info.rowStart;

  for (const cell of info.cells) {
    if (cell) {
      if (
        cell.startPosition &&
        (!rowStart || cell.startPosition.offset < rowStart.offset)
      ) {
        rowStart = cell.startPosition;
      }
      if (
        cell.endPosition &&
        (!rowEnd || cell.endPosition.offset > rowEnd.offset)
      ) {
        rowEnd = cell.endPosition;
      }
    }
  }

  // Create token for 'gtRow' with proper positioning
  const rowToken = {
    type: TYPE_ROW,
    start: rowStart || { line: 0, column: 0, offset: 0 },
    end: rowEnd || { line: 0, column: 0, offset: 0 },
  };
  ...
  for (const cell of info.cells) {
    if (cell) {
      const cellToken = {
        type: TYPE_CELL,
        start: cell.startPosition || { line: 0, column: 0, offset: 0 },
        end: cell.endPosition || { line: 0, column: 0, offset: 0 },
      };
