...
const toolbarItemIcons: Record<MiraEditorToolbarItem, MiraReactIcon> = {
  heading: Heading1,
  bold: Bold,
  italic: Italic,
...
export function MiraEditorToolbar({
  className,
  context,
  ...
  toolbars = [],
  value = "",
}: MiraEditorToolbarProps): React.ReactElement {
  ...
  const toolbarItems = useMemo(
    () => resolveMiraEditorToolbarItems({ featureConfigs, features }),
  ...
  const renderAction = useCallback(
    (action: MiraEditorToolbarAction, align: "start" | "end" = "start") => {
  ...
  return (
    <div
      className="mira-editor-toolbar__event-root"
      ref={rootRef}
      role="presentation"
    >
      <div
        ...
        className={cx(
          "mira-toolbar mira-editor-toolbar mira-editor__toolbar",
          className,
        )}
        role="toolbar"
      >
        ...
        {toolbarItems.length > 0 ? (
          <div
            aria-label="Insert"
            className="mira-editor__toolbar-section"
            role="group"
          >
        ...
        <div className="mira-editor-toolbar__spacer" />
            ...
            >
              ...
              {splitModeVisible ? (
                <button
                  ...
                  type="button"
                >
                  <Columns2 aria-hidden="true" className="mira-editor__icon" />
                </button>
              ) : null}
              {viewModeOverflowVisible ? (
                <ViewModeDropdown
...
function Separator(): React.ReactElement {
  return <div className="mira-separator--vertical mira-editor__separator" />;
}
...
function DropdownAction({
  ...
  action: MiraEditorToolbarDropdownAction;
  align: "start" | "end";
  dynamicBoolean: (
    value:
      | boolean
  ...
  isOpen: boolean;
  menuItemIcon: (item: MiraEditorToolbarMenuItem) => MiraReactIcon | undefined;
  onMenuItemRun: (item: MiraEditorToolbarMenuItem) => void;
  onToggle: () => void;
}): React.ReactElement {
  const Icon = action.icon;
  return (
...
function ViewModeToggle({
  ...
  resolvedDefaultEditMode: MiraEditorEditMode;
}): React.ReactElement {
  ...
  return (
    <div className="mira-react-dropdown">
      <button
        ...
        type="button"
      >
        <Ellipsis aria-hidden="true" className="mira-editor__icon" />
      </button>
      {isOpen ? (
        <div
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  aria-hidden="true"
                  className="mira-editor-toolbar__menu-icon"
                />
                <span>{item.label}</span>
                {item.checked ? (
                  <Check
                    aria-hidden="true"
                    className="mira-editor-toolbar__menu-check"
                  />
                ) : null}
              </button>
            );
...
function MenuItem({
  ...
  onRun: (item: MiraEditorToolbarMenuItem) => void;
}): React.ReactElement {
  ...
  return (
    ...
    >
      {Icon ? (
        <Icon aria-hidden="true" className="mira-editor-toolbar__menu-icon" />
      ) : null}
      <span>{item.label}</span>
      {item.shortcut ? (
...
function runMenuItem(
  item: MiraEditorToolbarMenuItem,
  actionContext: () => MiraEditorToolbarActionContext,
  dynamicBoolean: (
    value:
      | boolean
      | ((context: MiraEditorToolbarActionContext) => boolean)
